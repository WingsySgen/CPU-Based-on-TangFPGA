...
SG32D：基于Tang FPGA的简易CPU/框架。定长指令，哈佛结构，单周期设计，32位宽。将在2024年6月1日开源。
...
指令集主要可以分为四类，分别是运算指令，跳转指令，访存指令和扩展指令。
...
函数与中断指令包括：call、ret、int(仅32bit)、iret(仅32bit)。
...
iret不接受任何参数，指令解码器ID会拉高中断撤销信号来让中断控制器结束本次中断。编写中断函数时需要用iret返回。
...
此外，32bit的汇编器会把每条指令当成伪指令识别并展开，以实现最大程度地简洁化以及与8bit版本保持一致。比如对于程序的跳转，32bit版本的跳转地址是一个偏移量，从绝对地址转到相对于当前PC的偏移量的工作由汇编器完成。或者当一个立即数超过了12位时，展开生成该宽立即数将由汇编器完成。
...
目前的中断控制器支持64优先级中断，低级中断会被高级中断打断，中断号越低优先级越高。

- 0号 - 31号：软中断。
- 32号 - 47号：被核内部占用的硬中断。
- 48号 - 62号：外部中断。
- 63号：作为无中断时的默认中断号被CPU本身占用。

CPU向外部提供48号 - 62号共15个中断端口，其余被保留。
